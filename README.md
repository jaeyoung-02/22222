...
8. **연락 폼**
   - 이메일 전송 폼
   - FormSpree 또는 EmailJS 통합

## 커스터마이징 팁

1. **그라데이션 변경**: `body` 스타일의 `background` 속성 수정
2. **카드 둥글기**: `.card`의 `border-radius` 값 조정
3. **애니메이션 속도**: `@keyframes` 의 `animation-duration` 변경
4. **그림자 효과**: CSS 변수 `--shadow-*` 값 조정

## 배포 방법

이 웹사이트를 배포하려면 **Publish 탭**으로 이동하여 원클릭으로 배포하세요.
